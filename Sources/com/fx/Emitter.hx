 ...
 class Emitter extends Entity 
{
	...
	override private function onUpdate(dt:Float):Void 
	{
		...
		while (numAliveChildren() != currentCapacity())
		{
			...
			particle.reset(x+xRandom-xRandom*2*Math.random(),y+ yRandom-yRandom*2*Math.random(), minLife + (maxLife-minLife) * Math.random(), minVelocityX + (maxVelocityX-minVelocityX) * Math.random(), minVelocityY + (maxVelocityY-minVelocityY) * Math.random(), container,angularVelocityMin+(angularVelocityMax-angularVelocityMin)*Math.random(),minScale+(maxScale-minScale)*Math.random());
