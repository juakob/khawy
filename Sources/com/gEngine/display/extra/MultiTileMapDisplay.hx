...
import com.gEngine.display.extra.TileMapDisplay.Orientation;

class MultiTileMapDisplay extends Layer {
	...
	var orientations:Array<Array<Int>>;
	var tile:Sprite;
	...
	public function new(tileType:Sprite, widthInTiles:Int, heightInTiles:Int, tileWidth:Int, tileHeight:Int) {
		...
		for (i in 0...widthInTiles * heightInTiles) {
			tiles.push([-1]);
			orientations.push([cast Orientation.None]);
		}
	}

	public function getTile(depth:Int, indexX:Int, indexY:Int):Int {
		return tiles[indexX + widthInTiles * indexY][depth];
	}

	public function depth(indexX:Int, indexY:Int):Int {
		return tiles[indexX + widthInTiles * indexY].length;
	}

	public function setTile(depth:Int, indexX:Int, indexY:Int, value:Int, flipX:Bool = false, flipY:Bool = false, rotate:Bool = false) {
		setTile2(depth, indexX + widthInTiles * indexY, value, flipX, flipY);
	}

	public function setTile2(depth:Int, index:Int, value:Int, flipX:Bool = false, flipY:Bool = false, rotate:Bool = false) {
		tiles[index][depth] = value;
		...
		if (orientations.length <= depth) {
			orientations[index].push(tileOrientation);
		} else {
			orientations[index][depth] = tileOrientation;
		}
	}
	...
	override function render(paintMode:PaintMode, transform:FastMatrix4) {
		...
		mergeMinMax(paintMode.camera.screenToWorld(paintMode.camera.width, this.y), min, max);
		mergeMinMax(paintMode.camera.screenToWorld(paintMode.camera.width, this.y + paintMode.camera.height), min, max);
		mergeMinMax(paintMode.camera.screenToWorld(0, paintMode.camera.height), min, max);
		var startInTilesX = Std.int((min.x - this.x) / tileWidth) - 1;
		var endInTilesX = Std.int((max.x - this.x) / tileWidth) + 1;
		var startInTilesY = Std.int((min.y - this.y) / tileHeight) - 1;
		var endInTilesY = Std.int((max.y - this.y) / tileHeight) + 1;
		if (startInTilesX < 0)
			startInTilesX = 0;
		if (startInTilesY < 0)
			startInTilesY = 0;
		...
		for (y in startInTilesY...endInTilesY) {
			for (x in startInTilesX...endInTilesX) {
				...
				if (index >= 0 && index < tiles.length) {
					var counter:Int = 0;
					for (frame in tiles[index]) {
						var orientation = orientations[index][counter++];
						if (frame >= 0) {
							tile.timeline.gotoAndStop(frame);
							tile.x = this.x + x * tileWidth;
							tile.y = this.y + y * tileHeight;
