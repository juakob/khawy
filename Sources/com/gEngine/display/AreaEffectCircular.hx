...
class AreaEffectCircular implements IDraw
{
	...
	private  function createDrawFinishRectangle(paintMode:PaintMode,painter:IPainter,transform:FastMatrix4,realWidthRatio:Float,realHeightRatio:Float):Void
	{
		...
		for (i in 0...numSegments)
		{
			...
			writeVertexProjectionZoom(painter,tp.multvec(new FastVector4(centerX+leftDirection.x * outsideRadio,centerY+leftDirection.y * outsideRadio,z,1)),
											tp.multvec(new FastVector4(centerX+leftDirection.x * outsideRadio*zoom,centerY+leftDirection.y * outsideRadio*zoom,z,1)),
											realWidthRatio,realHeightRatio,invert);

			writeVertexProjectionZoom(painter,tp.multvec(new FastVector4(centerX+rigthDirection.x * outsideRadio,centerY+rigthDirection.y * outsideRadio,z,1)),
											tp.multvec(new FastVector4(centerX+rigthDirection.x * outsideRadio*zoom,centerY+rigthDirection.y * outsideRadio*zoom,z,1)),
											realWidthRatio,realHeightRatio,invert);
			...
			writeVertexProjection(painter,tp.multvec(new FastVector4(centerX+rigthDirection.x * insideRadio,centerY+rigthDirection.y * insideRadio,z,1)),
									realWidthRatio,realHeightRatio,invert);
			...
			writeVertexProjection(painter,tp.multvec(new FastVector4(centerX+leftDirection.x * outsideRadio,centerY+leftDirection.y * outsideRadio,z,1)),
									realWidthRatio,realHeightRatio,invert);

			writeVertexProjection(painter,tp.multvec(new FastVector4(centerX+rigthDirection.x * outsideRadio,centerY+rigthDirection.y * outsideRadio,z,1)),
									realWidthRatio,realHeightRatio,invert);

			writeVertexProjectionZoom(painter,tp.multvec(new FastVector4(centerX+leftDirection.x * insideRadio,centerY+leftDirection.y * insideRadio,z,1)),
												tp.multvec(new FastVector4(centerX+leftDirection.x * insideRadio*zoom,centerY+leftDirection.y * insideRadio*zoom,z,1)),
												realWidthRatio,realHeightRatio,invert);
			
			writeVertexProjectionZoom(painter,tp.multvec(new FastVector4(centerX+rigthDirection.x * insideRadio,centerY+rigthDirection.y * insideRadio,z,1)),
											tp.multvec(new FastVector4(centerX+rigthDirection.x * insideRadio*zoom,centerY+rigthDirection.y * insideRadio*zoom,z,1)),
											realWidthRatio,realHeightRatio,invert);
	...
	public inline static  function writeVertexProjectionZoom(painter:IPainter, vertex:FastVector4, uv:FastVector4,realWidthRatio:Float,realHeigthRatio:Float,invert:Int) {
		painter.write(vertex.x/vertex.w );
		painter.write(vertex.y/vertex.w );
		painter.write(vertex.z/vertex.w);
