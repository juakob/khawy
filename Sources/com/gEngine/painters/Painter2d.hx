...
class Painter2d implements IPainter
{
	...
	{	
		if (blend == null) blend = Blend.blendDefault();
		if (autoDestroy) PainterGarbage.i.add(this);
		initShaders(blend);
	...
	public function render(clear:Bool = false ,cropArea:MinMax=null ):Void
	{
		...
		// Begin rendering
		g.begin();
		if(cropArea!=null) g.scissor(Std.int(cropArea.min.x),Std.int(cropArea.min.y),Std.int(cropArea.max.x),Std.int(cropArea.max.y));
		uploadVertexBuffer();
		// Clear screen
		...
		buffer = downloadVertexBuffer();
		if(cropArea!=null) g.disableScissor();
		g.end();
