...
class TouchSliderH extends Entity {
	...
	override function onUpdate(dt:Float):Void {
		...
		if (startTouch && Input.i.isMouseDown()) {
			var mouseX:Float = Input.i.getMouseX();
			var mouseY:Float = Input.i.getMouseY();
			if (mouseX < right() && mouseX > left() && mouseY > top() && mouseY < bottom() && (Math.abs(startX - mouseX) > maxDelta || Math.abs
				(startY - mouseY) > maxDelta)) {
				startTouch = false;
		...
		if (scrollDirection == ScrollDirection.Horizontal) {
			if (display.x > scrollStart) {
				...
				slotVel = velocity = 0;
			} else if (scrollLimit > 0 && -display.x > scrollLimit) {
			...
			if (display.y > scrollStart) {
				...
				slotVel = velocity = 0;
			} else if (scrollLimit > 0 && -display.y > scrollLimit) {
