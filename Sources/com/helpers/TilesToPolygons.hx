...
class TilesToPolygons
{
	...
	static private function generateShape(rectangles:Array<RawTilesRectangel>,tileWidth:Float,tileHeight:Float,body:B2Body) :Void
	{
		for (rectangle in rectangles) 
		{
			...
			rectShape.setAsOrientedBox((rectangle.width * tileWidth) / 2, (rectangle.height * tileHeight) / 2, 
										new B2Vec2(rectangle.x * tileWidth + (rectangle.width * tileWidth) / 2, rectangle.y * tileHeight + (rectangle.height * tileHeight) / 2));
			def.shape =rectShape;
			def.density=0;
			def.friction=1;
		...
		var def:B2FixtureDef = new B2FixtureDef();
			var rectShape = new B2PolygonShape();
			rectShape.setAsVector([new B2Vec2(60*Const.invWorldScale,120*Const.invWorldScale),new B2Vec2(70*Const.invWorldScale,130*Const.invWorldScale)
